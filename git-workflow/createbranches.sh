...
function replaceProjectVersion() {
	...
	glpi-integration) $SCRIPTDIR/../replaceInFile.sh "<version>$ADDONS_GLPI_INTEGRATION_ORIGIN_VERSION</version>" "<version>$ADDONS_GLPI_INTEGRATION_TARGET_VERSION</version>" "pom.xml -not -wholename \"*/target/*\"" ;;
...
function replaceProjectDeps() {
	...
	SCRIPTDIR/../replaceInFile.sh "<version>$ADDONS_ADDONS_PARENT_ORIGIN_VERSION</version>" "<version>$ADDONS_ADDONS_PARENT_TARGET_VERSION</version>" "pom.xml -not -wholename \"*/target/*\"" ;;
}
...
function createFB() {
	...
	git commit -m "$ISSUE: Update projects versions/dependencies" -a
