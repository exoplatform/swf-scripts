...
for i in ${_REPOS}; do
  ...
  if [ -f "${repo}/pom.xml" ]; then
    maven_cmd() {
      sudo docker run --rm -v $(readlink -m ${repo}):/home -v /opt/prdacc/mavenpatch/settings.xml:/root/.m2/settings.xml --dns="8.8.8.8" --dns="8.8.8.4" --sysctl net.ipv6.conf.all.disable_ipv6=1 -w /home maven:3.9.9 mvn $*
    }
