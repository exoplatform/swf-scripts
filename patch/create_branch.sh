...
for i in ${_REPOS}; do
  ...
  curl -s -f -XPUT -L "https://api.github.com/repos/${ORGANIZATION}/${repo}/branches/patch%2F${tagversion}/protection" \
    ...
    --data '{ "required_status_checks":{"strict": true,"contexts": ["CI Build / CI Build"]},  "enforce_admins": true,  "required_pull_request_reviews": {"dismiss_stale_reviews": true,"required_approving_review_count": 1  },"restrictions": null}'
